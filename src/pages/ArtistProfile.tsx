  ...
  useEffect(() => {
    const fetchArtistData = async () => {
      ...
      const [{ data: profileData, error: profileError }, { data: contactRows }] = await Promise.all([
        supabase
          .from('profiles')
          .select('id, slug, first_name, last_name, stage_name, avatar_url, cover_url, cover_theme, bio, country, county, specialization, band_members, career_start_year, number_of_events, music_genres, instruments, estimated_price, facebook_url, instagram_url, youtube_url, tiktok_url, spotify_url, allow_promotion, plan, is_active, is_verified, created_at, updated_at')
          .eq('id', id)
          .maybeSingle(),
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="relative left-1/2 mb-6 w-screen -translate-x-1/2 md:left-0 md:mb-8 md:w-full md:translate-x-0">
                ...
                <div className="flex flex-col items-center justify-center text-center px-1">
                  <div className="flex items-center gap-1.5 md:gap-2">
                    ...
                    <span className="text-base md:text-2xl font-bold text-foreground">
                      {artist.created_at
                        ? new Date(artist.created_at).toLocaleDateString(i18n.language || 'en', { month: 'short', year: 'numeric' }).replace('.', '')
                        : '—'}
                    </span>
                  </div>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-5 mb-4 md:mb-8 h-auto p-1 md:p-1.5 gap-0.5 rounded-none md:rounded-[18px] -mx-4 md:mx-0 w-[calc(100%+2rem)] md:w-full bg-card dark:bg-[#111111] border-y md:border border-border dark:border-[#2A2A2A]">
              ...
              <>
                ...
                <div className="flex flex-wrap gap-2 md:gap-3">
                        {(() => {
                          ...
                          if (visibleLinks.length === 0) return <p className="text-muted-foreground text-sm">No social media links available.</p>;
                  ...
                  <div className="w-full max-w-[500px] mx-auto space-y-3 md:space-y-4">
                    ...
                    return combined.length > 0 ? combined.map((item) => {
                      if (item.type === 'promotion') {
                        ...
                        return (
                          <Card key={`promo-${promo.id}`} className="overflow-hidden shadow-sm my-0 border-solid rounded-none border-secondary bg-background border-0">
                      ...
                      return (
                        <Card key={post.id} className="overflow-hidden shadow-sm my-0 border-solid rounded-none border-secondary bg-background border-0">
                            {/* Header */}
                            <div className="p-4 pb-0 px-[6px] py-[3px]">
                              <div className="flex items-start justify-between">
                                <div className="flex items-center gap-3">
                ...
                <div className="-mx-4 md:mx-0 w-[calc(100%+2rem)] md:w-full">
                  <div className="w-full max-w-[500px] mx-auto space-y-3 md:space-y-4">
                      {announcements.filter((a) => !a.is_premium && !isAdExpired(a)).length > 0 ? announcements.filter((a) => !a.is_premium && !isAdExpired(a)).map((announcement) => <Card key={announcement.id} className="overflow-hidden shadow-sm my-0 border-solid rounded-none border-secondary bg-background border-0">
                            {/* Header */}
                            <div className="p-4 pb-0 px-[6px] py-[3px]">
                              <div className="flex items-start justify-between">
                                <div className="flex items-center gap-3">
