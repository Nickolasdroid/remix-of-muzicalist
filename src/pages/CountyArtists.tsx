...
const CountyArtists = () => {
  ...
  useEffect(() => {
    const fetchArtists = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('profiles').select('id, stage_name, avatar_url, plan, specialization').ilike('county', county);
      if (error) {
  ...
  return <div className="min-h-screen md:ml-64 bg-background">
      ...
      <div className="container mx-auto px-4 pt-20 md:pt-32 pb-24 md:pb-20">
        ...
        {loading ? <div className="text-center text-muted-foreground">Loading artists...</div> : <div className="space-y-10 md:space-y-16 max-w-7xl mx-auto">
            {categories.map(category => {
          const categoryArtists = getArtistsBySpecialization(category.key);
          if (categoryArtists.length === 0) return null;
          return <div key={category.key} className="space-y-4 md:space-y-8">
                  <div className="gap-3 md:gap-4 flex items-center justify-start">
                    ...
                    <Button variant="outline" size="icon" onClick={() => toggleViewMode(category.key)} className="border-accent text-accent hover:bg-accent hover:text-accent-foreground h-8 w-8 md:h-10 md:w-10">
                      {viewModes[category.key] === 'carousel' ? <List className="h-5 w-5" /> : <Grid className="h-5 w-5" />}
                    </Button>
                  </div>
                  
                  {viewModes[category.key] === 'carousel' ? <Carousel opts={{
              align: "start",
              loop: true
            }} className="w-full max-w-7xl mx-auto">
                      <CarouselContent>
                        {categoryArtists.map(artist => <CarouselItem key={artist.id} className="basis-1/2 md:basis-1/3 lg:basis-1/4">
                            <div className="p-2">
                              <ArtistProfileCard id={artist.id} stageName={artist.stage_name} imageUrl={artist.avatar_url} />
                            </div>
                          </CarouselItem>)}
                      </CarouselContent>
                      <CarouselPrevious className="left-0" />
                      <CarouselNext className="right-0" />
                    </Carousel> : <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-4xl mx-auto">
                ...
                return <Link key={artist.id} to={`/artist/${artist.id}`}>
                            <div className={`flex items-center gap-4 p-3 rounded-lg border ${borderColor} ${hoverBorderColor} ${hoverBgColor} transition-all duration-300`}>
                              <div className={`w-16 h-16 rounded-full overflow-hidden border-2 ${avatarBorderColor} flex-shrink-0`}>
                                {artist.avatar_url ? <img src={artist.avatar_url} alt={artist.stage_name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-accent/30 to-accent/10" />}
                              </div>
            ...
            {artists.length === 0 && <div className="text-center text-muted-foreground">
