-- Update handle_new_user to remove experience_level
...
BEGIN
  ...
  INSERT INTO public.profiles (
    ...
    specialization, career_start_year
  )
  VALUES (
    NEW.id, v_first_name, v_last_name, v_stage_name, NEW.email, v_phone,
    v_country, v_county, v_avatar, v_specialization, v_career_start
  )
...
-- Drop the column from profiles and pending_artist_registrations, then drop the enum
ALTER TABLE public.profiles DROP COLUMN IF EXISTS experience_level;
ALTER TABLE public.pending_artist_registrations DROP COLUMN IF EXISTS experience_level;
